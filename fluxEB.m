...
function [p,pcont,diverg]=fluxEB(element,coordinate,u,nodes2edge,edge2element)
p=zeros(3*size(element,1),2);
pcont = zeros(size(coordinate,1),2);
contador = zeros(size(coordinate,1),2);
for j=1:size(element,1)
  ...
  diverg(j) = 2*sum(diag(signum)*diag(n)*u(diag(nodes2edge(element(j,[2 3 1]),...
             element(j,[3 1 2])))))/det([1 1 1;coordinate(element(j,:),:)']);
         
  pcont(element(j,:),1) = pcont(element(j,:),1) + pc(1,:)';
  pcont(element(j,:),2) = pcont(element(j,:),2) + pc(2,:)';
  contador(element(j,:),1:2)= contador(element(j,:),1:2) + [1 1];
end
pcont = pcont./contador;
